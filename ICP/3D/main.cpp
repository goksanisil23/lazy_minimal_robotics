...
int main(int argc, char *argv[])
{
    ...
    program.add_argument("--input_pcd").help("input pcd file");
    program.add_argument("-visualize").help("visualize ICP registration").default_value(false).implicit_value(true);
    program.add_argument("-2d").help("use a simpler 2d curve").default_value(false).implicit_value(true);
    program.add_argument("-plane").help("use Point-to-Plane algorithm").default_value(false).implicit_value(true);
    ...
    Eigen::Matrix<float, 3, Eigen::Dynamic>::Map(target.at(0).data(), target_matrix.rows(), target_matrix.cols()) =
        target_matrix;
