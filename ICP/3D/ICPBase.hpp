...
{
  public:
    ICPBase(int16_t max_iterations_in, bool visualize_in)
        : max_iterations(max_iterations_in), visualize(visualize_in), input_cloud(new pcl::PointCloud<pcl::PointXYZ>),
          target_cloud(new pcl::PointCloud<pcl::PointXYZ>)
    {
        if (visualize)
        {
            ...
            vis->setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_COLOR, 1, 0, 0, std::string("input"));
            vis->setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_COLOR, 1, 1, 1, std::string("target"));
            vis->setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 7, std::string("input"));
            vis->setPointCloudRenderingProperties(
                pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, std::string("target"));
        }
    }
    ...
    Eigen::Matrix3f computeCrossCovar(const Eigen::MatrixXf      &input,
                                      const Eigen::MatrixXf      &target,
                                      const std::vector<int16_t> &correspondences)
    {
    ...
    void findCorrespondencesBruteForce(const Eigen::MatrixXf &input,
                                       const Eigen::MatrixXf &target,
                                       std::vector<int16_t>  &correspondences)
    {
    ...
    void findCorrespondencesKnn(const Eigen::MatrixXf &input,
                                const Eigen::MatrixXf &target,
                                std::vector<int16_t>  &correspondences)
    {
        // For each point in the input set, find the closest one in target set
        for (auto input_itr = input.colwise().begin(); input_itr != input.colwise().end(); input_itr++)
        {
            ...
            Mrpt::exact_knn(
                *input_itr,
                target,
                1,
                nn_indices.data()); // query_point, target_dataset, num_nn_neighbors, indices_of_found_neighbors
            correspondences.at(input_itr - input.colwise().begin()) = nn_indices(0);
        }
    }

    // TODO: This is super slow now.
    void drawCorrespondences(const Eigen::MatrixXf      &input,
                             const Eigen::MatrixXf      &target,
                             const std::vector<int16_t> &correspondences)
    {
        ...
        for (int i = 0; i < input.cols(); i++)
        {
            ...
            vis->addLine<pcl::PointXYZ>(pcl::PointXYZ(input_pt(0), input_pt(1), input_pt(2)),
                                        pcl::PointXYZ(target_pt(0), target_pt(1), target_pt(2)),
                                        std::string("correspondence") + std::to_string(i));
        }
