...
class STAPLE_TRACKER
{
  ...
  protected:
    ...
    void       initializeAllAreas(const cv::Mat &im, const cv::Size &target_size);

    void getSubwindow(const cv::Mat    &im,
                      cv::Point_<float> centerCoor,
                      cv::Size          model_sz,
                      cv::Size          scaled_sz,
                      cv::Mat          &output);
    void getSubwindowFloor(const cv::Mat    &im,
                           cv::Point_<float> centerCoor,
                           cv::Size          model_sz,
                           cv::Size          scaled_sz,
                           cv::Mat          &output);
  ...
  private:
    ...
    cv::Point_<float> bbox_center;
    cv::Size          target_sz;

    cv::Size bg_area_;
    cv::Size fg_area_;
    double   area_resize_factor_;
    cv::Size cf_response_size_;

    cv::Size norm_bg_area_;
    cv::Size norm_target_sz_;
    cv::Size norm_delta_area_;
    cv::Size norm_pwp_search_area_;
