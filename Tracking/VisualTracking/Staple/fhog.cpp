...
void gradMag(float *I, float *M, float *O, int h, int w, int d, bool full)
{
    ...
    for (x = 0; x < w; x++)
    {
        ...
        for (y = 0; y < h4 / 4; y++)
        {
            _m     = sse::MIN2(sse::RCPSQRT(_M2[y]), sse::SET(1e10f));
            _M2[y] = sse::RCP(_m);
            if (O)
                _Gx[y] = sse::MUL(sse::MUL(_Gx[y], _m), sse::SET(acMult));
            if (O)
                _Gx[y] = sse::XOR(_Gx[y], sse::AND(_Gy[y], sse::SET(-0.f)));
        };
        ...
        if (O != 0 && full)
        {
            ...
            for (; y < h - 4; y += 4)
                sse::STRu(O[y + x * h],
                          sse::ADD(sse::LDu(O[y + x * h]),
                                   sse::AND(sse::CMPLT(sse::LDu(Gy[y]), sse::SET(0.f)), sse::SET(PI))));
            for (; y < h; y++)
                O[y + x * h] += (Gy[y] < 0) * PI;
        }
    }
...
void gradMagNorm(float *M, float *S, int h, int w, float norm)
{
    ...
    if (sse)
        for (; i < n4; i++)
        {
            *_M = sse::MUL(*_M, sse::RCP(sse::ADD(*_S++, _norm)));
            _M++;
        }
    if (sse)
        i *= 4;
                  ...
                  bool   interpolate)
{
    ...
    if (interpolate)
        for (i = 0; i <= n - 4; i += 4)
        {
            _o     = sse::MUL(sse::LDu(O[i]), _oMult);
            _o0    = sse::CVT(_o);
            _od    = sse::SUB(_o, sse::CVT(_o0));
            _o0    = sse::CVT(sse::MUL(sse::CVT(_o0), _nbf));
            _o0    = sse::AND(sse::CMPGT(_oMax, _o0), _o0);
            *_O0++ = _o0;
            _o1    = sse::ADD(_o0, _nb);
            _o1    = sse::AND(sse::CMPGT(_oMax, _o1), _o1);
            *_O1++ = _o1;
            _m     = sse::MUL(sse::LDu(M[i]), _norm);
            *_M1   = sse::MUL(_od, _m);
            *_M0++ = sse::SUB(_m, *_M1);
            _M1++;
        }
        ...
        {
            _o     = sse::MUL(sse::LDu(O[i]), _oMult);
            _o0    = sse::CVT(sse::ADD(_o, sse::SET(.5f)));
            _o0    = sse::CVT(sse::MUL(sse::CVT(_o0), _nbf));
            _o0    = sse::AND(sse::CMPGT(_oMax, _o0), _o0);
            *_O0++ = _o0;
            *_M0++ = sse::MUL(sse::LDu(M[i]), _norm);
            *_M1++ = sse::SET(0.f);
            *_O1++ = sse::SET(0);
        }
...
#define GH(H, ma, mb)                                                                                                  \
    H1 = H;                                                                                                            \
    sse::STRu(*H1, sse::ADD(sse::LDu(*H1), sse::MUL(ma, mb)));
    ...
    for (int i = 1; i < *h_height - dh; i++)
        for (int j = 1; j < *h_width - dw; j++)
            for (int k = 0; k < depth; k++)
                crop_H[i - 1 + (j - 1) * (crop_h) + k * (crop_h * crop_w)] =
                    H[k * (*h_width * *h_height) + j * (*h_height) + i];
    delete H;
    *h_height = crop_h;
    *h_width  = crop_w;
    return crop_H;
