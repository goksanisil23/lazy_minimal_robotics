...
// optimize
template <typename T>
T evaluateModelAtSamplePoint(const T &m_est, const T &c_est,
                             const double &x_gt)
{
                   ...
                   std::vector<double> &y_est)
{
    ...
    for (double x = SAMPLE_INTERVAL.first; x <= SAMPLE_INTERVAL.second;
         x += SAMPLE_SPACING)
    {
        y_est.at(idx) = evaluateModelAtSamplePoint(m_est, c_est, x);
        idx++;
    }
}
...
public:
    ...
    void
    oplusImpl(const double *update) override
    {
...
public:
    ...
    bool operator()(const T *params, T *residual) const
    {
        residual[0] = T(_measurement) -
                      evaluateModelAtSamplePoint(params[0], params[1], _x_sample);
        return true;
    }
    ...
    {
        const CurveFittingVertex *current_estimates = static_cast<const CurveFittingVertex *>(_vertices[0]);
        // double *curr_m_est = (double *)_vertices[0];
        std::cout << "curr est " << current_estimates->estimate() << std::endl;
        _error[0] = evaluateModelAtSamplePoint<double>(current_estimates->estimate()[0], current_estimates->estimate()[1], _x_sample) - _measurement;
    }
#endif
...
int main()
{
    ...
    for (bool use_robuts_kernel : robust_kernel_usage)
    {
        ...
        for (int obs_idx = 0; obs_idx < N_SAMPLES; obs_idx++)
        {
            ...
            edge->setId(obs_idx);
            // connect the 0th (only) vertex of THIS EDGE to a given vertex
