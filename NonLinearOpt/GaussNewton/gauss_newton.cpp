// This is a sample example of Gauss-Newton method, where we're trying to estimate the non-linear function
// y = exp(a*x^2 + b*x + c) + w, where w is the Gaussian measurement noise.
...
Eigen::MatrixXd residual(N_SAMPLES,1); // container where we store the error btw GT and model
Eigen::MatrixXd J(N_SAMPLES, N_PARAM); // Jacobian but concatanated at each sample

///////////////////////////////////

// The real function we're trying to approximate
double evaluateGT(const double& x)
{
...
int main()
{
    ...
    // Get all the observation samples from the GT equation
    int idx = 0; 
