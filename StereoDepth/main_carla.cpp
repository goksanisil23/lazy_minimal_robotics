...
constexpr bool ENABLE_POST_FILTERING = true;
...
int main()
{
    ...
    while (std::getline(std::cin, line)) // until space is pressed
    {
        ...
        auto disparityLeftSGBM =
            stereo_depth.computeLeftDisparityMapSGBM(cv_img_left, cv_img_right, ENABLE_POST_FILTERING);
        auto depthMap = stereo_depth.computeDepthFromLeftDisparityMap(disparityLeftSGBM);
        // A) Use Stereo-depth for 3D projection
        stereo_depth.projectLeftImgTo3D(cv_img_left, depthMap);
        // B) Use ground truth depth for 3D projection
