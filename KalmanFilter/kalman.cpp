...
KalmanFilter::KalmanFilter(const Eigen::MatrixXd &A,
    ...
    : A(A), B(B), C(C), Q(Q), R(R), P_prio(P0), P_post(P0), n(A.rows()), m(C.rows()), c(B.cols()), I(n, n),
      initialized(false)
{
    I.setIdentity();
}
...
void KalmanFilter::prediction_update(const Eigen::VectorXd &u)
{
    if (!initialized)
    {
...
void KalmanFilter::innovation_update(const Eigen::VectorXd &y)
{
