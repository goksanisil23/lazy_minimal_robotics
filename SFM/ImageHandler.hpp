...
{
  public:
    CarlaImageHandler(const std::string &depthFilesDir, const std::string &rgbFilesDir)
    {
...
{
  public:
    NumpyImageHandler(const std::string &depthFilesDir, const std::string &rgbFilesDir)
    {
        for (const auto &file : std::filesystem::directory_iterator(depthFilesDir))
        {
            if (file.path().string().find("depth_") != std::string::npos)
    ...
    cv::Mat getDepthImage(const std::string &depthImagePath)
    {
        ...
        for (size_t ii = 0; ii < imageHeight; ii++)
        {
            for (size_t jj = 0; jj < imageWidth; jj++)
            {
                depthImage.at<double>(ii, jj) = static_cast<double>(npyDataPtr[jj + ii * imageWidth]);
