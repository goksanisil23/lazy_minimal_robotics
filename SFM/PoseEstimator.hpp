...
class PoseEstimator
{
  public:
                                      ...
                                      std::vector<Eigen::Vector3d>   &landmark_points)
    {
        for (auto pt_in_cam_frame : model_points)
        {
            Eigen::Vector3d pt_in_world_frame =
                Eigen::Vector3d(-pt_in_cam_frame.x, -pt_in_cam_frame.y, pt_in_cam_frame.z);
        ...
        if (!descriptors.empty() && !prev_descriptors_.empty())
        {
            ...
            for (auto el : knn_matches)
            {
                ...
                if (el[0].distance < LOWE_MATCH_RATIO * el[1].distance)
                {
                    ...
                    meanPixelDistance += calculateMeanPixelDist(good_prev_keypoints.back(), good_keypoints.back());
                    ...
                    double depth =
                        prev_depth_img_.at<double>(good_prev_keypoints.back().y, good_prev_keypoints.back().x);
                    if ((depth > 0) && (depth < DEPTH_THRESHOLD))
                    {
                        ...
                        auto mf_coords = MatchedFeatureCoords(good_prev_keypoints.back().x,
                                                              good_prev_keypoints.back().y,
                                                              good_keypoints.back().x,
                                                              good_keypoints.back().y);
            ...
            // if ((image_points.size() > MIN_MATCHES) && (meanPixelDistance > MIN_MEAN_PIXEL_DISTANCE))
            {
                ...
                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> eigenR(
                    R_total.ptr<double>(), R_total.rows, R_total.cols);
                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> eigenR_prev(
                    R_prev.ptr<double>(), R_prev.rows, R_prev.cols);

                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> eigenT(
                    t_total.ptr<double>(), t_total.rows, t_total.cols);
                Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> eigenT_prev(
                    t_prev.ptr<double>(), t_prev.rows, t_prev.cols);
    ...
    void saveCameraPose(const int &imgIdx, const Eigen::Vector3d &t, const Eigen::Matrix3d &R)
    {
        ...
        camera_poses_txt_ << imgIdx << " " << t(0) << " " << t(1) << " " << t(2) << " " << quat.x() << " " << quat.y()
                          << " " << quat.z() << " " << quat.w() << std::endl;
    }

    double calculateMeanPixelDist(const cv::Point2f &pt1, const cv::Point2f &pt2)
    {
    ...
    void stepPnpKlt(cv::Mat &rgb_image, cv::Mat &depth_image)
    {
        ...
        if (prev_keypoints_klt_.size() > 0)
        {
            ...
            cv::calcOpticalFlowPyrLK(prev_img_,
                                     ...
                                     3,
                                     klt_termination_criteria_);
            cv::calcOpticalFlowPyrLK(img_gray,
                                     prev_img_,
                                     tracked_keypoints,
                                     backtracked_prev_keypoints,
                                     status,
                                     err,
                                     cv::Size(KLT_WIN_SIZE, KLT_WIN_SIZE),
                                     KLT_PYRAMIDS,
            ...
            {
                auto of_verif_dist = cv::norm(backtracked_prev_keypoints.at(i) - prev_keypoints_klt_.at(i));
                if ((status[i] == 1) && (of_verif_dist < 1.0)) // if the flow for the corresponding feature is found
                {
                    ...
                    double depth = prev_depth_img_.at<double>(prev_keypoints_klt_.at(i).y, prev_keypoints_klt_.at(i).x);
                    if (depth > 0)
                    {
                        ...
                        image_points.push_back(cv::Point2d(tracked_keypoints[i].x,
                                                           tracked_keypoints[i].y)); // corresponding image point at (k)
                        cv::line(rgb_image, tracked_keypoints[i], prev_keypoints_klt_.at(i), cv::Scalar(0, 0, 255), 3);
                    }
                }
            }
        ...
        {
            cv::goodFeaturesToTrack(img_gray, prev_keypoints_klt_, MAX_CORNERS, 0.3, 7, cv::Mat(), 7);
        }
