...
int main(int argc, char *argv[]) {
  try {
    ...
    if(argc == 2)
      transform = map->GetRecommendedSpawnPoints()[std::stoi(argv[1])];
    ...
    VISO::Tracking viso_tracking(getCameraIntrinsic(camera_rgb_bp));
    ...
    for(int ii = 0; ii < NUM_SIM_STEPS; ii++)
    {
      ...
      if(ii > 30) // wait until the vehicle is moving a bit
      {
        ...
        // VISO
        viso_tracking.stepPnP(cv_img, cv_depth_img); 
      } 
    }
