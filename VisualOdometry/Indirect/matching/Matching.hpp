...
class VO
{
  ...
  public:
    VO(cv::Mat intrinsics_K)
        : K_(intrinsics_K), c_x_(K_.at<float>(0, 2)), c_y_(K_.at<float>(1, 2)), f_x_(K_.at<float>(0, 0)),
          f_y_(K_.at<float>(1, 1))
    {
    ...
    void stepEssentialMatrixDecomp(cv::Mat &rgb_image, cv::Mat &depth_image)
    {
        ...
        {
            flann_matcher_->knnMatch(prev_descriptors,
                                     descriptors,
                                     knn_matches,
                                     2); // find 2 best matches, with distance in increasing order
            ...
            cv::Mat E, mask;
            E = cv::findEssentialMat(good_prev_keypoints,
                                     good_keypoints,
                                     K_.at<float>(0, 0),
                                     cv::Point2d(K_.at<float>(0, 2), K_.at<float>(1, 2)),
                                     cv::RANSAC,
                                     0.999,
                                     0.5);
            cv::recoverPose(E,
    ...
    void stepPnP(cv::Mat &rgb_image, cv::Mat &depth_image)
    {
        ...
        if (!descriptors.empty() && !prev_descriptors.empty())
        {
                ...
                if (el[0].distance < LOWE_MATCH_RATIO * el[1].distance)
                {
                    ...
                    if (depth < 999.0)
                    {
                        ...
                        image_points.push_back(cv::Point2d(
                            good_keypoints.back().x, good_keypoints.back().y)); // corresponding image point at (k)
                    }
                }
            }
            cv::Mat img_matches;
            ...
            cv::Mat R_rod;
            cv::solvePnPRansac(
                model_points, image_points, K_, cv::Mat::zeros(4, 1, cv::DataType<double>::type), R_rod, t_current);
    ...
    void updateAbsScale(const float &x_truth, const float &y_truth, const float &z_truth)
    {
        ...
        {
            abs_scale_   = std::sqrt((x_truth - x_truth_prev) * (x_truth - x_truth_prev) +
                                   (y_truth - y_truth_prev) * (y_truth - y_truth_prev) +
                                   (z_truth - z_truth_prev) * (z_truth - z_truth_prev));
            x_truth_prev = x_truth;
            y_truth_prev = y_truth;
            z_truth_prev = z_truth;
        }
    ...
    cv::Mat eulerAnglesToRotationMatrix(const cv::Vec3f &theta)
    {
        ...
        cv::Mat R_x = (cv::Mat_<double>(3, 3) << 1,
                       0,
                       0,
                       0,
                       std::cos(theta[0]),
        ...
        cv::Mat R_y = (cv::Mat_<double>(3, 3) << std::cos(theta[1]),
                       0,
                       std::sin(theta[1]),
                       0,
                       1,
        ...
        cv::Mat R_z = (cv::Mat_<double>(3, 3) << std::cos(theta[2]),
                       -std::sin(theta[2]),
                       0,
                       std::sin(theta[2]),
                       std::cos(theta[2]),
