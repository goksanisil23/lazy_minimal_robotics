...
class HungarianOptimizer
{
   ...
   * kHungarianOptimizerColNotFound if no such column exists. */
    ...
    /* Is row 'row' covered? */
    bool RowCovered(const size_t row) const
    {
        return rows_covered_[row];
    }

    /* Cover row 'row'. */
    void CoverRow(const size_t row)
    {
        rows_covered_[row] = true;
    }

    /* Uncover row 'row'. */
    void UncoverRow(const size_t row)
    {
        rows_covered_[row] = false;
    }

    /* Is column col covered? */
    bool ColCovered(const size_t col) const
    {
        return cols_covered_[col];
    }

    /* Cover column col. */
    void CoverCol(const size_t col)
    {
        cols_covered_[col] = true;
    }

    /* Uncover column col. */
    void UncoverCol(const size_t col)
    {
