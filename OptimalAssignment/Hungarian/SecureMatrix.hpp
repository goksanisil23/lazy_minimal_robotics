...
class SecureMat
{
  ...
  public:
    SecureMat()
    {
        Reserve(max_height_, max_width_);
    }
   ...
   * @return nothing */
    ...
    // Assumes row-major input vector that represents a matrix
    void AssignFromVec(std::vector<T> &costsIn)
    {
        for (size_t row = 0; row < height_; row++)
        {
            for (size_t col = 0; col < width_; col++)
            {
                mat_(row, col) = costsIn.at(col + row * width_);
            }
        }
    }
